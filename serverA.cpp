...
// map of struct GRAPH to hold all possibile mapIDs
map<char, GRAPH> graphs;
...
void readMap (string fileName) {
	...
	while (!fileIn.eof()) {
		...
		if (isalpha(c)) {
			if (isalpha(prevID)) {
				GRAPH prev = {c, speedProp, speedTrans, preGraph};
				graphs[prevID] = prev;
			}
			prevID = c;
			speedProp = 0;
...
/********************************************+*** main loop ******************************************************/ 
	while (1) {
		...
		cout << "The Server A has received input for finding graph of map " << mapID_quired << endl;
		...
		if (graphs.find(mapID_quired) == graphs.end()) {
			mapInfoToAWS.foundFlag = -1;
			cout << "The Server A does not have the required graph id" << mapID_quired << endl;
			if ((numbytes = sendto(sockfd, &mapInfoToAWS, MAX_LEN, 0, (struct sockaddr *)&their_addr, addr_len)) == -1) {
			...
			mapInfoToAWS.foundFlag = 0;	// map found!
			if ((numbytes = sendto(sockfd, &mapInfoToAWS, MAX_LEN, 0, (struct sockaddr *)&their_addr, addr_len)) == -1) {
				perror("senderr: sendto");
				exit(1);
			}
			cout << "The Server A has sent Graph to AWS" << endl;
		}
	}
	return 0;
}