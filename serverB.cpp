...
// copy from Beej's-Guide --listener.c
	...
	if ((rv = getaddrinfo(LOCALHOST, SERVER_B_UDP_PORT, &hints, &servinfo)) != 0) {
...
// copy from Beej's Guide --end
	...
	//main loop: 
	while (1) {
		// Server B receives the quired map ID form AWS via UDP

		// Server B looks up the map ID and return the map's information to AWS via UDP
	}
